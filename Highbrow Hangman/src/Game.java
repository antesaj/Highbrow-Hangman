...
public class Game {
	...
	// Panes
	private StackPane phraseNode;
	private TilePane phraseUScore, phrasePane, letters;
	private BorderPane gamePane;

	private static HangmanSound loseSound;
	private static HangmanSound gameSound;
	...
	public Game(String choice) {
		...
		count = 6;
		this.init();
		loseSound = new HangmanSound("losehangman.mp3");
		gameSound = new HangmanSound("gameSound.mp3");
		gameSound.play();
	}

	/*
	 * init() initializes the GUI for the game. It creates a new window for Hangman
	 * gameplay.
	 */
	...
	public void init() {
		...
		switch (choice) {
		case "Classic Novel Titles":
			phrase = dictionary1.getTitle();
			break;

		case "Star Wars Names":
			phrase = dictionary2.getName();
			break;
		}
	...
	/*
	 * setButtonStyle will edit the style of the passed Button. This is specifically
	 * for the letter buttons for the bottom of the screen.
	 */

	public static void setButtonStyle(Button x) {
		/*
		 * Media sound = new Media(new File("losehangman.mp3").toURI().toString());
		 * MediaPlayer mediaPlayer = new MediaPlayer(sound); HangmanSound loseSound =
		 * new HangmanSound("losehangman.mp3");
		 */
		x.setMinHeight(56);
		x.setMinWidth(56);
		...
		}); // End letter-clicked event handler
	}
